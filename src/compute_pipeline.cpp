...
ComputePipeline::ComputePipeline(
    ...
    vk_check(vkCreateShaderModule(context_.device(), &smci, nullptr, &shader),
    ...
    try {
        ...
        vk_check(vkCreateDescriptorSetLayout(
                     context_.device(), &dlci, nullptr, &descriptor_set_layout_),
        ...
        vk_check(vkCreatePipelineLayout(context_.device(), &plci, nullptr, &pipeline_layout_),
        ...
        vk_check(vkCreateComputePipelines(
                     context_.device(), VK_NULL_HANDLE, 1, &cpci, nullptr, &pipeline_),
        ...
        vk_check(vkCreateDescriptorPool(context_.device(), &dpci, nullptr, &descriptor_pool_),
        ...
        vk_check(vkAllocateDescriptorSets(context_.device(), &dsai, &descriptor_set_),
        ...
        vk_check(vkCreateCommandPool(context_.device(), &pool_ci, nullptr, &command_pool_),
        ...
        vk_check(vkAllocateCommandBuffers(context_.device(), &cbai, &command_buffer_),
                 "vkAllocateCommandBuffers failed");

        VkFenceCreateInfo fci{VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};
        vk_check(vkCreateFence(context_.device(), &fci, nullptr, &fence_),
        ...
        if (context_.capabilities().timestamp_valid_bits != 0) {
            ...
            qpci.queryCount = 2;
            vk_check(vkCreateQueryPool(context_.device(), &qpci, nullptr, &query_pool_),
...
DispatchStats ComputePipeline::run(
    ...
    vk_check(vkResetFences(context_.device(), 1, &fence_), "vkResetFences failed");
    vk_check(vkResetCommandBuffer(command_buffer_, 0), "vkResetCommandBuffer failed");

    VkCommandBufferBeginInfo begin{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};
    begin.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
    vk_check(vkBeginCommandBuffer(command_buffer_, &begin), "vkBeginCommandBuffer failed");
    ...
    vk_check(vkEndCommandBuffer(command_buffer_), "vkEndCommandBuffer failed");
    ...
    vk_check(vkQueueSubmit(context_.compute_queue(), 1, &submit, fence_),
    ...
    if (query_pool_ != VK_NULL_HANDLE) {
        ...
        const std::uint32_t bits = context_.capabilities().timestamp_valid_bits;
        ...
        stats.gpu_ms =
            (static_cast<double>(delta) *
             static_cast<double>(context_.capabilities().timestamp_period_ns)) /
            1'000'000.0;
