...
void Q8MatVecPipeline::run_staged_triplet(
    ...
    const bool command_buffers_changed =
        descriptors_changed ||
        triplet_bound_staging_input_ !=
            staging_input.handle() ||
        triplet_bound_staging_output_ !=
            staging_output.handle();
    ...
    if (descriptors_changed) {
        ...
        triplet_bound_weights_ =
            weights.handle();
        triplet_bound_input_ =
            input.handle();
        triplet_bound_output_ =
            output.handle();
        triplet_bound_output_dims_ =
            output_dims;
        triplet_descriptors_valid_ =
            true;
    }
    ...
    if (const auto it =
            triplet_command_cache_.find(key);
        it != triplet_command_cache_.end()) {
        command =
            it->second;
    } else {
        VkCommandBufferAllocateInfo allocate{};
        ...
        check(
            vkAllocateCommandBuffers(
                ...
                &allocate,
                &command),
            "vkAllocateCommandBuffers failed for cached Q8 triplet");
        ...
        try {
            ...
            check(
                vkBeginCommandBuffer(
                    command,
                    &begin),
                "vkBeginCommandBuffer failed for cached Q8 triplet");
            ...
            check(
                vkEndCommandBuffer(
                    command),
                "vkEndCommandBuffer failed for cached Q8 triplet");

            triplet_command_cache_.emplace(
                key,
                command);
        } catch (...) {
            if (command !=
